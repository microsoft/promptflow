...
pfazure run create --flow . --data ./data.jsonl --environment-variables AZURE_OPENAI_API_KEY='${open_ai_connection.api_key}' AZURE_OPENAI_API_BASE='${open_ai_connection.api_base}' --column-mapping text='${data.text}' --stream --runtime tmp-example-runtime-ci
# run using yaml file
pfazure run create --file run.yml --stream --runtime tmp-example-runtime-ci
```
