...
class FetchAndSummarizeExecutor(Executor):
    ...
    @handler
    async def process(self, url: str, ctx: WorkflowContext[dict]) -> None:
        text_content = _fetch_text_content_from_url(url)
        response = await self._agent.run(
            f"Text: {text_content}\nSummary:",
            options=OpenAIChatOptions(temperature=0.2, max_tokens=128),
        )
        await ctx.send_message({"url": url, "summary": response.text})
...
class ClassifyExecutor(Executor):
    ...
    @handler
    async def classify(self, data: dict, ctx: WorkflowContext[Never, dict]) -> None:
        url = data["url"]
