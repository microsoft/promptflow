...
.PARAMETER registry
The container registry name, for example 'xx.azurecr.io' or 'docker.io'.
.PARAMETER name
The app name to produce a unique FQDN as AppName.azurewebsites.net.
.PARAMETER location
The app location, default to 'westcentralus'.
.PARAMETER sku
...
.EXAMPLE
PS> .\deploy.ps1 -i <image_tag> -r <registry> -n <app_name> -g <resource_group>
.EXAMPLE
PS> .\deploy.ps1 -i <image_tag> -r <registry> -n <app_name> -g <resource_group> -Subscription "xxxx-xxxx-xxxx-xxxx-xxxx" -Verbose
#>
[CmdletBinding()]
param(
    [Alias("i", "image_tag")][string]$ImageTag,
    [Alias("r")][string]$Registry,
    [Alias("n")][string]$Name = "basic-with-connection8c78f9",
    [Alias("l")][string]$Location = "westcentralus",
    [string]$Sku = "F1",
