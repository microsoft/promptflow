...
def new_process(context, executor_creation_func):
    ...
    process.start()
    return process.pid, input_queue, output_queue
...
def _persist_and_convert_images_to_path_dicts(value, inplace=False):
...
def _process_multimedia_in_run_info(run_info: Union[FlowRunInfo, NodeRunInfo]):
    ...
    if run_info.inputs:
        run_info.inputs = _persist_and_convert_images_to_path_dicts(run_info.inputs)

    # Persist and convert images in output to path dictionaries.
    # This replaces any image objects with their corresponding file path dictionaries.
    if run_info.output:
        serialized_output = _persist_and_convert_images_to_path_dicts(run_info.output)
    ...
    if run_info.api_calls:
        run_info.api_calls = _persist_and_convert_images_to_path_dicts(run_info.api_calls, inplace=True)
    return run_info


def _process_multimedia_in_flow_run(run_info: FlowRunInfo):
    _process_multimedia_in_run_info(run_info)


def _process_multimedia_in_node_run(run_info: NodeRunInfo):
    run_info = _process_multimedia_in_run_info(run_info)
    return run_info


def _process_multimedia(result: LineResult, output_dir) -> LineResult:
    ...
    if not output_dir:
        return result
    _process_multimedia_in_flow_run(result.run_info)
    for node_name, node_run_info in result.node_run_infos.items():
        result.node_run_infos[node_name] = _process_multimedia_in_node_run(node_run_info)
    result.output = persist_multimedia_data(result.output, output_dir)
    return result
...
def monitor_process(
    ...
    global is_exit  # define a global variable
    while not is_exit:
        try:
            args = task_queue.get(timeout=1)
        except queue.Empty:
            end_process_by_pid(process_id)
            return

        input_queue.put(args)
        inputs, line_number, run_id = args[:3]
        # self._processing_idx[line_number] = format_current_process(line_number)

        start_time = datetime.utcnow()
        completed = False

        while not is_exit and datetime.utcnow().timestamp() - start_time.timestamp() <= line_timeout_time:
            try:
                ...
                if isinstance(message, LineResult):
                    completed = True
                    message = _process_multimedia(message, output_dir)
                    # result_list.append(message)
                    result_queue.put(message)
                    break
                elif isinstance(message, FlowRunInfo):
                    storage.persist_flow_run(message)
                elif isinstance(message, NodeRunInfo):
                    storage.persist_node_run(message)
            except queue.Empty:
                continue
...
def manage_threads(
    ...
    threads = []
    for _, (process_id, input_queue, output_queue) in enumerate(processes_info_list):
...
class LineExecutionProcessPool:
    ...
    def run(self, batch_inputs):
        ...
        for _ in range(self._n_process):
            self.processes_info_list.append(new_process(self.context, self._executor_creation_func))

        # Start a new process for managing threads
        thread_manager_process = self.context.Process(
            target=manage_threads,
            args=(
                self._inputs_queue,
                result_queue,
                self.processes_info_list,
                self._line_timeout_sec,
                self._storage,
                self._output_dir,
            ),
        )

        thread_manager_process.start()
        result_list = []

        while 1:
            while not result_queue.empty():
                result_list.append(result_queue.get())
            if not thread_manager_process.is_alive():
                break
