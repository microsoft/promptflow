...
class ToolResolver:
    def __init__(
        self, working_dir: Path, connection_manager: Optional[ConnectionManager] = None
    ):
        self._package_tools = collect_package_tools()
        self._working_dir = working_dir
        self._connection_manager = (
            ConnectionManager() if connection_manager is None else connection_manager
        )
    ...
    def resolve_tool_by_node(
        ...
        if node.type is ToolType.PYTHON:
            ...
            raise NotImplementedError(
                f"Tool source type {node.source.type} is not supported yet."
            )
    ...
    def _load_source_content(self, source: ToolSource) -> str:
        if (
            ...
            or not Path(self._working_dir / source.path).exists()
        ):
    ...
    def _resolve_script_node(
        ...
        if node.source.path is None:
            raise UserErrorException(
                f"Node {node.name} does not have source path defined."
            )
