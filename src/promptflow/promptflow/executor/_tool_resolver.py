...
class ToolResolver:
    ...
    def _convert_to_connection_value(
        self, k: str, v: InputAssignment, node: Node, conn_types: List[ValueType]
    ):
        connection_value = self._connection_manager.get(v.value)
        if not connection_value:
            raise ConnectionNotFound(
                f"Connection {v.value} not found for node {node.name!r} input {k!r}."
            )
    ...
    def _convert_to_assistant_definition(
        self, assistant_definition_path: str, input_name: str, node_name: str
    ):
        if (
            assistant_definition_path is None
            or not (self._working_dir / assistant_definition_path).is_file()
        ):
            raise InvalidSource(
    ...
    def _convert_node_literal_input_types(
        self, node: Node, tool: Tool, module: types.ModuleType = None
    ):
        updated_inputs = {
            k: v
            for k, v in node.inputs.items()
            if (v.value is not None and v.value != "")
            or v.value_type != InputValueType.LITERAL
        }
        for k, v in updated_inputs.items():
            ...
            if ConnectionType.is_connection_class_name(value_type):
                ...
                try:
                    updated_inputs[k].value = self._convert_to_assistant_definition(
                        v.value, k, node.name
                    )
                except Exception as e:
                    error_type_and_message = f"({e.__class__.__name__}) {e}"
    ...
    def resolve_tool_by_node(
        self, node: Node, convert_input_types=True
    ) -> ResolvedTool:
        try:
            if node.source is None:
                raise UserErrorException(
                    f"Node {node.name} does not have source defined."
                )

            if node.type is ToolType.PYTHON:
                if node.source.type == ToolSourceType.Package:
                    return self._resolve_package_node(
                        node, convert_input_types=convert_input_types
                    )
                elif node.source.type == ToolSourceType.Code:
                    return self._resolve_script_node(
                        node, convert_input_types=convert_input_types
                    )
            ...
            if isinstance(e, PromptflowException) and e.target != ErrorTarget.UNKNOWN:
                raise ResolveToolError(
                    node_name=node.name, target=e.target, module=e.module
                ) from e
    ...
    def _load_source_content(self, node: Node) -> str:
        ...
        if (
            ...
            or not (self._working_dir / source.path).is_file()
        ):
            raise InvalidSource(
                target=ErrorTarget.EXECUTOR,
    ...
    def _validate_duplicated_inputs(
        self, prompt_tpl_inputs: list, tool_params: list, msg: str
    ):
    ...
    def _load_images_for_prompt_tpl(
        self, prompt_tpl_inputs_mapping: dict, node_inputs: dict
    ):
        for input_name, input in prompt_tpl_inputs_mapping.items():
            if ValueType.IMAGE in input.type and input_name in node_inputs:
                if node_inputs[input_name].value_type == InputValueType.LITERAL:
                    node_inputs[input_name].value = create_image(
    ...
    def _resolve_prompt_node(self, node: Node) -> ResolvedTool:
        ...
        self._validate_duplicated_inputs(
            prompt_tpl_inputs_mapping.keys(), param_names, msg
        )
        node.inputs = self._load_images_for_prompt_tpl(
            prompt_tpl_inputs_mapping, node.inputs
        )
    ...
    def _resolve_llm_node(self, node: Node, convert_input_types=False) -> ResolvedTool:
        ...
        updated_node = copy.deepcopy(node)
        updated_node.inputs[key] = InputAssignment(
            value=connection, value_type=InputValueType.LITERAL
        )
        ...
        self._validate_duplicated_inputs(
            prompt_tpl_inputs_mapping.keys(), tool.inputs.keys(), msg
        )
        updated_node.inputs = self._load_images_for_prompt_tpl(
            prompt_tpl_inputs_mapping, updated_node.inputs
        )
        ...
        api_func = (
            partial(api_func, **{prompt_tpl_param_name: prompt_tpl})
    ...
    def _resolve_llm_connection_to_inputs(self, node: Node, tool: Tool) -> Node:
        ...
        raise InvalidConnectionType(
            message_format="Connection type can not be resolved for tool {tool_name}",
            tool_name=tool.name,
        )

    def _resolve_script_node(
        self, node: Node, convert_input_types=False
    ) -> ResolvedTool:
        ...
        return ResolvedTool(
            node=node, definition=tool, callable=callable, init_args=init_args
        )

    def _resolve_package_node(
        self, node: Node, convert_input_types=False
    ) -> ResolvedTool:
        ...
        self._remove_init_args(updated_node.inputs, init_args)
        return ResolvedTool(
            node=updated_node, definition=tool, callable=callable, init_args=init_args
        )
    ...
    def _integrate_prompt_in_package_node(self, resolved_tool: ResolvedTool):
        ...
        self._validate_duplicated_inputs(
            prompt_tpl_inputs_mapping.keys(),
            resolved_tool.definition.inputs.keys(),
            msg,
        )
        node.inputs = self._load_images_for_prompt_tpl(
            prompt_tpl_inputs_mapping, node.inputs
        ...
        resolved_tool.callable = partial(
            callable, **{prompt_tpl_param_name: prompt_tpl}
        )
