...
from promptflow.sdk._orm.connection import Connection as ORMConnection
from promptflow.sdk._utils import decrypt_secret_value, encrypt_secret_value, find_type_in_override, in_jupyter_notebook
from promptflow.sdk.entities._yaml_translatable import YAMLTranslatableMixin
from promptflow.sdk.schemas._connection_schema import (
...
class _Connection(YAMLTranslatableMixin):
    ...
    def _validate_and_encrypt_secrets(self):
        scrubbed = [k for k, v in self.secrets.items() if v == SCRUBBED_VALUE]
        if scrubbed:
            raise ValueError(f"Connection {self.name!r} secrets {scrubbed} must be filled again when updating it.")
        return {k: encrypt_secret_value(v) for k, v in self.secrets.items()}
    ...
    def _load(
        ...
        **kwargs,
        ...
        context = {
            BASE_PATH_CONTEXT_KEY: Path(yaml_path).parent if yaml_path else Path("../../azure/entities/"),
...
class AzureOpenAIConnection(_StrongTypeConnection):
    ...
    def __init__(
        self, api_key: str, api_base: str, api_type: str = "azure", api_version: str = "2023-03-15-preview", **kwargs
    ):
...
class OpenAIConnection(_StrongTypeConnection):
    ...
    def __init__(self, api_key: str, organization: str, **kwargs):
...
class QdrantConnection(_StrongTypeConnection):
...
class CustomConnection(_Connection):
    ...
    def _from_orm_object_with_secrets(cls, orm_object: ORMConnection):
        ...
        secrets = {
            k: decrypt_secret_value(orm_object.connectionName, v["value"])
            for k, v in json.loads(orm_object.customConfigs).items()
            if v["configValueType"] == ConfigValueType.SECRET.value
        }
