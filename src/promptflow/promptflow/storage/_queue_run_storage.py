...
class ServiceQueueRunStorage(QueueRunStorage):
    ...
    def persist_node_run(self, run_info: NodeRunInfo):
        ...
        MultimediaProcessor.get_instance().process_multimedia_in_run_info(run_info, node_folder)

    def persist_flow_run(self, run_info: FlowRunInfo):
        super().persist_flow_run(run_info)
        flow_folder = self._flow_artifacts_path / str(run_info.index)
        MultimediaProcessor.get_instance().process_multimedia_in_run_info(run_info, flow_folder)
