...
from promptflow.storage import AbstractRunStorage, AbstractCacheStorage
...
class AbstractCacheManager:
    ...
    def get_cache_result(
            self,
            cache_info: CacheInfo) -> CacheResult:
...
class CacheManager(AbstractCacheManager):
    ...
    def get_cache_result(
        ...
        try:
            ...
            flow_logger.warning(f'Failed to get cached run result. \
                            Run id:{cached_run_info.run_id}, flow run id: {cached_run_info.flow_run_id} \
                            Exception: {ex}')
            return None

        flow_logger.info(f'Hit cached result of previous run: run id: \
