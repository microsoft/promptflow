...
from ..contracts.flow import InputValueType, ToolSource, ToolSourceType
from ..contracts.tool import Tool, ToolType
...
from ..utils.generate_tool_meta_utils import generate_prompt_tool, generate_python_tool
from ..utils.tool_utils import function_to_tool_definition, get_prompt_param_name_from_func
...
class BuiltinsManager:
    ...
    def load_builtin(
        ...
        """Load built in tool with given config."""
        # TODO:
        # 1 Follow the latest tool meta definition
        # 2 Avoid loading the same builtin tool multiple times
        module_name, class_name, method_name = self.parse_builtin_tool_method(tool)
        ...
        for k, v in node_inputs.items():
            ...
            if v.value_type != InputValueType.LITERAL:
                raise ValueError(
                    f"Input {k!r} of tool {tool.name!r} only supports literal values, got {v.serialize()!r}"
                )
        ...
        if missing_inputs:
            raise MissingRequiredInputs(
                message=f"Required inputs {list(missing_inputs)} are not provided for tool {tool.name!r}.",
                target=ErrorTarget.EXECUTOR,
            )
    ...
    @staticmethod
    def _parse_config_class(provider_class: type):
        if not hasattr(provider_class, "from_config"):
            raise NotImplementedError(f"Provider {provider_class} does not have from_config method.")
        parameters = inspect.signature(provider_class.from_config).parameters
        if "config" not in parameters:
            raise NotImplementedError(
                f"Provider {provider_class} does not have config parameter in from_config method."
            )
        config_class = parameters["config"].annotation
        from ..connections import CustomConnection

        if not is_dataclass(config_class) and config_class != CustomConnection:
            raise NotImplementedError(f"Non-dataclass config {config_class} is not supported.")
        return config_class
