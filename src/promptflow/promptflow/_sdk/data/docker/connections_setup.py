...
def update_migration_secret(migration_secret: str):
    keyring.set_password(
        "promptflow",
        "encryption_key",
        base64.urlsafe_b64encode(hashlib.sha256(migration_secret.encode("utf-8")).digest()).decode("utf-8"),
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--migration-secret", type=str, help="migration secret used for the db")
    parser.add_argument(
    ...
    if not args.migration_secret and args.migration_secret_file:
        if not os.path.isfile(args.migration_secret_file):
            raise FileNotFoundError(f"migration secret file {args.migration_secret_file} not found.")
        with open(args.migration_secret_file, "r") as f:
            args.migration_secret = f.read()

    if not args.migration_secret:
        raise ValueError("either migration-secret or migration-secret-file should be provided")
