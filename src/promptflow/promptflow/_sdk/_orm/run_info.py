...
class RunInfo(Base):
    ...
    def archive(self) -> None:
        ...
        self.archived = True
        with mgmt_db_session() as session:
            session.query(RunInfo).filter(RunInfo.name == self.name).update(
    ...
    @sqlite_retry
    def list(
        max_results: Optional[int], list_view_type: ListViewType
    ) -> List["RunInfo"]:
        with mgmt_db_session() as session:
            basic_statement = session.query(RunInfo)
            # filter by archived
            list_view_type = (
                list_view_type.value
                if isinstance(list_view_type, Enum)
                else list_view_type
            )
            if list_view_type == ListViewType.ACTIVE_ONLY.value:
                basic_statement = basic_statement.filter(
                    RunInfo.archived == False
                )  # noqa: E712
            elif list_view_type == ListViewType.ARCHIVED_ONLY.value:
                basic_statement = basic_statement.filter(
                    RunInfo.archived == True
                )  # noqa: E712
