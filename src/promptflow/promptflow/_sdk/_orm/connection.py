...
class Connection(Base):
    ...
    customConfigs = Column(
        TEXT, nullable=False
    )  # For strong type connection, custom configs is an empty dict
    createdDate = Column(
        TEXT, nullable=False
    )  # ISO8601("YYYY-MM-DD HH:MM:SS.SSS"), string
    ...
    def create_or_update(connection: "Connection") -> None:
        ...
        try:
            ...
            update_dict = {
                k: v for k, v in connection.__dict__.items() if not k.startswith("_")
            }
            update_dict.pop("createdDate")
            session.query(Connection).filter(Connection.connectionName == name).update(
                update_dict
            )
            session.commit()
    ...
    def get(name: str, raise_error=True) -> "Connection":
        with mgmt_db_session() as session:
            connection = (
                ...
                .filter(Connection.connectionName == name)
                .first()
            )
    ...
    @sqlite_retry
    def list(
        max_results: Optional[int] = None, all_results: bool = False
    ) -> List["Connection"]:
        with mgmt_db_session() as session:
            if all_results:
                ...
                return [
                    run_info
                    for run_info in session.query(Connection).limit(max_results)
                ]
