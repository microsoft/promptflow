...
def assign_tool_input_index_for_ux_order_if_needed(tool):
    ...
    if should_preserve_tool_inputs_order(tool_type) and "inputs" in tool:
        ...
        for input_name, settings in inputs_dict.items():
            # 'uionly_hidden' means the inputs are only on the user side and will not be shown on the main interface.
            # Skip 'uionly_hidden' input type because these inputs will be shown in a popup window and they are passed
            # to ux as a list which order is same with the generated by func parameters. No need the 'ui_hints: index'.
