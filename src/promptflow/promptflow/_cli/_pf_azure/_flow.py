...
def add_parser_flow_create(subparsers):
    ...
    create_parser = subparsers.add_parser(
        "create", description="Create a flow for promptflow.", help="pf flow create"
    )
...
def add_parser_flow_list(subparsers):
    ...
    list_parser = subparsers.add_parser(
        "list", description="List flows for promptflow.", help="pf flow list"
    )
...
def create_flow(
    ...
    flow_operations = _get_flow_operation(
        subscription_id, resource_group, workspace_name
    )
    flow_draft = flow_operations._create_or_update(flow_entity)
    print(
        f"Successfully created flow {flow_draft.flow_name!r} with flow id {flow_draft.flow_id!r}."
    )
    return flow_draft
...
def list_flows(
    ...
    if flow_count > 0:
        ...
        for flow in flows:
            print(
                f"Name: {flow.name!r}, owner: {flow.owner!r}, flow_id: {flow.flow_id!r}"
            )
...
def download_flow(
    ...
    print(
        f"Successfully download flow from file share path {source!r} to {destination!r}."
    )
