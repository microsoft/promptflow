...
def add_parser_run(subparsers):
    ...
    run_parser = subparsers.add_parser(
        "run",
        description="A CLI tool to manage cloud runs for prompt flow.",
        help="Manage promptflow runs.",
    )
...
def add_parser_run_stream(subparsers):
    ...
    run_stream_parser.add_argument(
        "-n", "--name", type=str, required=True, help="The run name to stream."
    )
...
def add_parser_run_show(subparsers):
    ...
    run_show_parser = subparsers.add_parser(
        "show", description="A CLI tool to show a run.", help="Show a run."
    )
    _set_workspace_argument_for_subparsers(run_show_parser)
    run_show_parser.add_argument(
        "-n", "--name", type=str, required=True, help="The run name to show."
    )
...
def add_parser_run_show_metrics(subparsers):
    ...
    run_show_metrics_parser = subparsers.add_parser(
        ...
        help="Show run metrics.",
    )
    _set_workspace_argument_for_subparsers(run_show_metrics_parser)
    run_show_metrics_parser.add_argument(
        "-n", "--name", type=str, required=True, help="The run name to show metrics."
    )
...
def add_parser_run_cancel(subparsers):
    ...
    run_cancel_parser = subparsers.add_parser(
        "cancel", description="A CLI tool to cancel a run.", help="Cancel a run."
    )
    _set_workspace_argument_for_subparsers(run_cancel_parser)
    run_cancel_parser.add_argument(
        "-n", "--name", type=str, required=True, help="The run name to cancel."
    )
...
def add_parser_run_archive(subparsers):
    ...
    run_archive_parser.add_argument(
        "-n", "--name", type=str, required=True, help="The run name to archive."
    )
...
def add_parser_run_restore(subparsers):
    ...
    run_restore_parser.add_argument(
        "-n", "--name", type=str, required=True, help="The run name to restore."
    )
...
def add_parser_run_update(subparsers):
    ...
    run_update_parser.add_argument(
        "-n", "--name", type=str, required=True, help="The run name to update."
    )
    ...
    if args.sub_action == "create":
        ...
        create_run(
            create_func=functools.partial(
                pf.runs.create_or_update, runtime=args.runtime
            ),
            args=args,
        )
        ...
        show_metrics(
            args.subscription, args.resource_group, args.workspace_name, args.name
        )
...
def show_run_details(
    ...
    pf = _get_azure_pf_client(
        subscription_id, resource_group, workspace_name, debug=debug
    )
...
def stream_run(
    subscription_id, resource_group, workspace_name, flow_run_id, debug=False
):
