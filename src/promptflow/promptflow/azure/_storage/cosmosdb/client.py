...
def get_client_with_workspace_info(
        container_name: str,
        workspace_info: dict) -> ContainerProxy:
    
    subscription_id = workspace_info.get("subscription_id", None)
    resource_group_name = workspace_info.get("resource_group_name", None)
...
def get_client(
        ...
        workspace_name: str) -> ContainerProxy:
    ...
    if container_client is None:
        with _thread_lock:
            ...
            if container_client is None:
                ...
                client_map[client_key] = {"expire_at": datetime.datetime.now() + datetime.timedelta(0, _token_timeout), "client": container_client}
...
def _get_resource_token(
    ...
    token_resp = pfclient._traces._get_cosmos_db_token(container_name=container_name, acquire_write=True)
