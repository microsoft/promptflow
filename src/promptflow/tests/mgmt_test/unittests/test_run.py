...
from promptflow.sdk._constants import BASE_PATH_CONTEXT_KEY, NODES
from promptflow.sdk._load_functions import load_run
from promptflow.sdk._pf_client import PFClient
from promptflow.sdk._run_functions import create_yaml_run, run
from promptflow.sdk.entities import Run
from promptflow.sdk.operations._run_submitter import RunSubmitter, overwrite_variant, variant_overwrite_context
...
class TestRun:
    ...
    def test_load_run(self):
        input_dict = {
            "data": (FLOWS_DIR / "webClassification1.jsonl").resolve().as_posix(),
    ...
    def test_dot_env_resolve(self):
        ...
        source = f"{FLOWS_DIR}/runs/sample_bulk_run.yaml"
    ...
    @pytest.mark.parametrize(
        "source, error_msg",
        [
            (f"{FLOWS_DIR}/runs/illegal/extra_field.yaml", "Unknown field"),
            (f"{FLOWS_DIR}/runs/illegal/non_exist_data.yaml", "Can't find directory or file"),
        ],
    )
    ...
    def test_run_bulk_invalid_params(self):
        # Test if function raises FileNotFoundError
        with pytest.raises(FileNotFoundError):
            run(flow="invalid_path", data="fake_data")

        with pytest.raises(FileNotFoundError):
            run(flow="invalid_path", data="fake_data", batch_run="fake_run")
  ...
  node1:
    variants:
      variant1:
        ...
        # Test if function raises ValueError
    ...
    @patch("promptflow.sdk.operations._run_submitter.RunSubmitter._run_bulk")
    @patch("promptflow.sdk.operations._run_operations.RunOperations.update")
    def test_submit(self, mock_run_create, mock_update):
        ...
        # Check if Run.create method was called
