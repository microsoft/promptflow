...
async def oai_assistant(conn: OpenAIConnection, content: PromptTemplate, assistant_id: str, assistant_definition: AssistantDefinition):
    ...
    invoker = DefaultToolInvoker.load_assistant_tools(assistant_definition.tools)
    ...
    while run.status != "completed":
        ...
        if run.status == "requires_action":
            ...
            for tool_call in tool_calls:
                output = invoker.invoke_assistant_tool(tool_call.function.name, json.loads(tool_call.function.arguments))
                tool_outputs.append({
