...
# create run
pfazure run create --flow . --data ./data.jsonl --column-mapping url='${data.url}' --stream --runtime tmp-example-runtime-ci 
...
# create run using yaml file, --name is optional
pfazure run create --file run.yml --runtime tmp-example-runtime-ci --name $run_name
...
pfazure run create --flow ../../evaluation/eval-classification-accuracy --data ./data.jsonl --column-mapping groundtruth='${data.answer}' prediction='${run.outputs.category}' --run $prev_run_name --runtime tmp-example-runtime-ci
pfazure run create --file run_evaluation.yml --run $prev_run_name --stream --name $run_name --runtime tmp-example-runtime-ci
