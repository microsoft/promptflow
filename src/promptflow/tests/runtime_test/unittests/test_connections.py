...
import pytest
from endpoint_test.conftest import PROMOTFLOW_ROOT

from promptflow.runtime.connections import build_connection_dict_from_rest_object
from promptflow.runtime.models import WorkspaceConnectionPropertiesV2BasicResource
from promptflow.runtime.utils._utils import decode_dict, encode_dict

from .._utils import get_config_file


@pytest.mark.unittest
def test_encode_decode_connections():
    template_dict = json.loads(open(get_config_file("connections.json.example", root=PROMOTFLOW_ROOT), "r").read())
    encoded = encode_dict(template_dict)
    assert isinstance(encoded, str)
    decoded = decode_dict(encoded)
    assert isinstance(decoded, dict)
    assert template_dict == decoded


def build_from_data_and_assert(data, expected):
    obj = WorkspaceConnectionPropertiesV2BasicResource.deserialize(data)
    assert build_connection_dict_from_rest_object("mock", obj) == expected
...
@pytest.mark.unittest
def test_build_bing_category_connection_from_rest_object():
    # Test on CustomKeys type with Bing category
    data = {
        "id": "mock_id",
        ...
        "properties": {
            ...
            "category": "CustomKeys",
            "target": "https://gpt-test-eus.openai.azure.com/",
            "metadata": {"azureml.flow.connection_type": "Bing", "azureml.flow.module": "promptflow.connections"},
        },
    }
    expected = {"type": "BingConnection", "module": "promptflow.connections", "value": {"api_key": "***"}}
    build_from_data_and_assert(data, expected)
