...
class TestConnectionUtils:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (MyCustomConnectionMissingTypeComments,
             ["api_key: to_replace_with_api_key  # The api key."]),
            (MyCustomConnectionMissingParamComments,
             ["api_key: to_replace_with_api_key  # String"]),
        ]
    )
    def test_generate_custom_strong_type_connection_template_with_comments(
            self,
            cls,
            expected_str_in_template):
        package = "test-package"
        package_version = "0.0.1"
        spec = generate_custom_strong_type_connection_spec(cls, package, package_version)
        template = \
            generate_custom_strong_type_connection_template(cls, spec, package, package_version)
    ...
    def test_generate_custom_strong_type_connection_template_with_default_value(self):
        package = "test-package"
        package_version = "0.0.1"
        spec = generate_custom_strong_type_connection_spec(
            MyCustomConnectionWithDefaultValue,
            package,
            package_version)
