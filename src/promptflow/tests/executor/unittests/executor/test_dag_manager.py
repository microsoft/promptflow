...
class TestDAGManager:
    ...
    def test_pop_bypassed_nodes_with_exception(self):
        ...
        error_message = (
            "The node 'node2' referenced by 'node3' has been bypassed, so the value of this node cannot be returned. "
            "Please refer to the node that will not be bypassed as the default return value."
        )
