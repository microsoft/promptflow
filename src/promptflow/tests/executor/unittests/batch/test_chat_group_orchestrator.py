...
class TestChatGroupOrchestrator:
    ...
    @pytest.mark.parametrize(
        ...
        [(InvalidChatRoleCount, "Invalid chat group role count: 1. Please define 2 chat group roles at least.")],
    )
    ...
    @pytest.mark.parametrize(
        "error_code, error_message",
        [(InvalidMaxTurnValue, "Invalid max_turn value for chat group run: 0. Please assign max_turn at least 1.")],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            (get_chat_role(role="user", flow_file=get_yaml_file("hello-world")), get_conversation_history(empty=True)),
            (
    ...
    def test_process_flow_outputs(self, chat_role, conversation_history):
        ...
        orchestrator = ChatGroupOrchestrator(
            [get_chat_role(role="user", flow_file=get_yaml_file("hello-world")), chat_role], 3
        )
    ...
    @pytest.mark.parametrize(
        "error_code, error_message",
        [(UsingReservedRoleKey, "chat role output use reserved key role")],
    )
