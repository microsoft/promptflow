...
@pytest.mark.e2etest
def test_serving_api_with_encoded_connection(flow_serving_client_with_encoded_connection):
...
@pytest.mark.e2etest
def test_serving_api_with_prt_config_env(flow_serving_client_with_prt_config_env):
...
@pytest.mark.e2etest
def test_serving_api_with_conn_provider_env(flow_serving_client_with_connection_provider_env):
...
@pytest.mark.usefixtures("flow_serving_client_with_connection_provider_env")
@pytest.mark.e2etest
def test_serving_api_with_aml_resource_id_env(flow_serving_client_with_aml_resource_id_env):
...
@pytest.mark.e2etest
def test_serving_api_with_connection_name_override(serving_client_with_connection_name_override):
...
@pytest.mark.e2etest
def test_serving_api_with_connection_data_override(serving_client_with_connection_data_override):
    response = serving_client_with_connection_data_override.get("/health")
    assert b'{"status":"Healthy","version":"0.0.1"}' in response.data

    response = serving_client_with_connection_data_override.post("/score", data=json.dumps({"text": testdata}))
    assert (
        response.status_code == 200
    ), f"Response code indicates error {response.status_code} - {response.data.decode()}"
    assert "output_prompt" in json.loads(response.data.decode())
