...
class RecordFile:
    ...
    def set(self, record_file_input) -> None:
        ...
        if not self._record_file.parts[-1].endswith(RecordFile._standard_record_name):
            record_folder = self._record_file / RecordFile._standard_record_folder
            self._record_file = record_folder / RecordFile._standard_record_name
        else:
    ...
    def load_file(self, cached_items) -> None:
...
class RecordCache:
    ...
    def get_cache(self, record_file: Union[str, Path]) -> object:
        self.record_file = RecordFile()
        self.record_file.set(Path(record_file))
    ...
    def get_record(self, input_dict: Dict) -> object:
        ...
        current_saved_records = self.cached_items.get(self.record_file.load_file(), None)
        if current_saved_records is None:
            raise RecordFileMissingException(f"Record file not found {self.record_file.get()}.")
        saved_output = current_saved_records.get(hash_value, None)
        if saved_output is None:
            raise RecordItemMissingException(
                f"Record item not found in file {self.record_file.get()}.\n" f"values: {json.dumps(input_dict)}\n"
            )
