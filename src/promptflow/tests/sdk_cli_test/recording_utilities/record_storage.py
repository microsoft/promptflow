...
class RecordFile:
    ...
    def set_and_load_file(self, record_file_input, cached_items):
        ...
        if not self.real_file.parts[-1].endswith(RecordFile._standard_record_name):
            record_folder = self.real_file / RecordFile._standard_record_folder
            self.real_file = record_folder / RecordFile._standard_record_name
        else:
            record_folder = self.real_file.parent
...
class RecordCache:
    ...
    def get_record(self, input_dict: Dict) -> object:
        ...
        output = line_record["output"]
        output_type = line_record["output_type"]
