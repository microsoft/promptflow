...
class TestCli:
    ...
    def test_node_run_telemetry(self, local_client):
        ...
        def assert_node_run(*args, **kwargs):
            record = args[0]
            assert record.msg.startswith("pf.flow.node_run") or record.msg.startswith("pf.flows.node_run")
            assert record.custom_dimensions["activity_name"] in ["pf.flow.node_run", "pf.flows.node_run"]
