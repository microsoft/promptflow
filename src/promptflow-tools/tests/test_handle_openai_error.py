...
from promptflow.connections import AzureOpenAIConnection, SerpConnection, OpenAIConnection
...
@pytest.fixture
def serp_connection() -> SerpConnection:
    return ConnectionManager().get("serp_connection")


@pytest.fixture
def aoai_provider(azure_open_ai_connection) -> AzureOpenAI:
    aoai_provider = AzureOpenAI.from_config(azure_open_ai_connection)
    return aoai_provider


@pytest.mark.usefixtures("use_secrets_config_file", "aoai_provider", "azure_open_ai_connection", "serp_connection")
class TestHandleOpenAIError:
    ...
    def test_aoai_authencation_error_with_api_key(self, azure_open_ai_connection):
        """
                     bad api key

        API                           Error                    Message
        completion,embedding,chat     AuthenticationError      The same
        """
        ...
        with pytest.raises(WrappedOpenAIError) as exc_info:
            completion(azure_open_ai_connection, prompt=prompt_template, deployment_name="text-ada-001")
        ...
        with pytest.raises(WrappedOpenAIError) as exc_info:
            embedding(azure_open_ai_connection, input=prompt_template, deployment_name="text-embedding-ada-002")
        assert error_msg == exc_info.value.message
        assert error_code == ErrorResponse.from_exception(
            exc_info.value).error_code_hierarchy

    def test_aoai_authencation_error_with_api_base(self, azure_open_ai_connection):
        """
             bad api base (endpoint)

        API                             Error                   Message
        completion,embedding, chat      APIConnectionError      The same
    ...
    def test_aoai_invalid_request_error_with_api_version(self, azure_open_ai_connection):
        """
             bad api version

        API                             Error                   Message
        completion,embedding, chat      InvalidRequestError     the same
        ...
        # Embedding throw: Exception occurs: InvalidRequestError: The API deployment for this resource does not exist.
    ...
    def test_aoai_invalid_request_error_api_type(self, azure_open_ai_connection):
        """
             bad api type

        API                             Error                   Message
        completion,embedding, chat      InvalidAPIType     the same
    ...
    def test_aoai_invalid_request_error_with_deployment(self, aoai_provider, azure_open_ai_connection):
        """
        ...
        with pytest.raises(WrappedOpenAIError) as exc_info:
            aoai_provider.completion(prompt=prompt_template, deployment_name=deployment)
        assert error_msg == exc_info.value.message
        assert error_code == ErrorResponse.from_exception(
            exc_info.value).error_code_hierarchy
        ...
        with pytest.raises(WrappedOpenAIError) as exc_info:
            embedding(azure_open_ai_connection, input=prompt_template, deployment_name=deployment)
        assert error_msg == exc_info.value.message
        assert error_code == ErrorResponse.from_exception(
            exc_info.value).error_code_hierarchy
    ...
    def test_retriable_openai_error_handle(self, azure_open_ai_connection, mocker: MockerFixture, dummyExceptionList):
    ...
    def test_invalid_connection_type(self, serp_connection):
