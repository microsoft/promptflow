...
def _calculate_mean(df) -> Dict[str, float]:
    df.rename(columns={col: col.replace(_OUTPUTS, "") for col in df.columns}, inplace=True)
...
def _validate_columns(
    ...
    """
    if target:
        if any(c.startswith(_OUTPUTS) for c in df.columns):
            raise ValueError(f'The column cannot start from "{_OUTPUTS}" if target was defined.')
        ...
        for map_to_key, map_value in mapping_config.items():
            ...
            if match is not None:
                ...
                if pattern.startswith(pattern_prefix):
                    ...
                    # Target-generated columns alwauys starts from .outputs.
                    map_from_key = f"{_OUTPUTS}{pattern[len(run_outputs_prefix) :]}"
...
def _rename_columns_conditionally(df: pd.DataFrame, target_generated: Set[str]):
    ...
    for col in df.columns:
        ...
        # Do not rename columns with _OUTPUTS
        if _OUTPUTS in col and col[len(_OUTPUTS) :] in target_generated:
            continue
