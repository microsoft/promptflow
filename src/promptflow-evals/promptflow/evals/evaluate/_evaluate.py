...
def evaluate(
    ...
    **kwargs,
    ...
    :keyword evaluator_config: Configuration for evaluators. The configuration should be a dictionary with evaluator
        names as keys and a dictionary of input mappings as values. The column mappings should be a dictionary with
        keys as the parameters of the callable evaluator, and values as the keys in the input json data or the output 
	dictionary generated by target. For example, evaluator_config = {"question": "${data.chat_input"} means that 
	"question" is a required parameter for evaluator callable, and "chat_input" is a key in the jsonl input data. 
	Similarly, if the key "chat_input" is in the output dictionary of target callable, use "${target.chat_input"}. 
	Users only need to specify this mapping if the keys are different. For example, {"question": "${data.question"} 
	or {"question": "${target.question}"} are not necessary.
