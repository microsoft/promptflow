        ...
        *,
        ...
        for template in templates:
            for parameter in template.template_parameters:
                ...
                if len(tasks["jailbreak"]) + len(tasks["regular"]) >= max_simulation_results:
                    break
            if len(tasks["jailbreak"]) + len(tasks["regular"]) >= max_simulation_results:
                break

        sim_results = {"jailbreak": [], "regular": []}
        for task_type, task_list in tasks.items():
            for task in asyncio.as_completed(task_list):
                sim_results[task_type].append(await task)
                progress_bar.update(1)

        return JsonLineList(sim_results["jailbreak"]), JsonLineList(sim_results["regular"])
