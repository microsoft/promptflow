...
from promptflow._sdk._utility.serve_utils import _resolve_python_flow_additional_includes


@pytest.mark.unittest
def test_flow_serve_resolve_additional_includes():
    # Assert flow path not changed if no additional includes
    flow_path = (PROMPTFLOW_ROOT / "tests/test_configs/flows/web_classification").resolve().absolute().as_posix()
    ...
    flow_path = (
        (PROMPTFLOW_ROOT / "tests/test_configs/flows/web_classification_with_additional_include")
        .resolve()
        .absolute()
        .as_posix()
    )
